...
void character_builder::ask_for_race_and_name() const {
  ...
  const std::vector<std::string> race_options{"Human", "Dwarf", "Elf",
											  "Halfling"};
...
void character_builder::build_attributes(const std::string &name,
										 const std::string &label,
										 const race_options race) const {
  ...
  result_->add_component<attributes_component>(
	  label, name, race, strength, dexterity,
...
void character_builder::ask_for_job() const {
  ...
  const std::vector<std::string> job_options{"Lumberer", "Blacksmith",
											 "Farmer"};
...
std::shared_ptr<entity> character_builder::build_character() {
  ...
  if (gender == 0)
	name =
		name_list::male_names.at((rand() % name_list::male_names.capacity()));
  if (gender == 1)
	name = name_list::female_names.at(
		(rand() % name_list::female_names.capacity()));

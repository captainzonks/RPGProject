...
void npc_info_menu::handle_events(game *game) {
  if (choice_ != 0) {
	switch (choice_) {
	  case 1:entity_manager::instance()->add_entity(character_builder::instance()->build_character());
		break;
	  case 2:entity_manager::instance()->display_entities();
		break;
	  case 3:std::cout << entity_manager::instance()->get_entity_count() << std::endl;
		break;
	  case 4:game->pop_state();
		break;
	  default:break;
	}
  }

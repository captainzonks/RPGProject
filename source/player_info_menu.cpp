...
void player_info_menu::handle_events(game *game) {
  if (choice_ != 0) {
	switch (choice_) {
	  case 1: game->player->display();
		break;
	  case 2: game->player->get_component<inventory_component>()->display_inventory();
		game->player->get_component<inventory_component>()->display_equipped();
		break;
	  case 3: game->player->get_component<currency_component>()->display_money();
		break;
	  case 4: game->player->list_all_components();
		break;
	  case 5: game->pop_state();
		break;
	  default: break;
	}
  }
